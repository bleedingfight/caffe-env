...
def convert_dataset(output_res, dataset_store, shape=(225, 225)):
    ...
    image_mean = findfile(caffe_home, 'compute_image_mean')
    ...
    if image_mean is not None:
        output_mean = join(dataset_store,'mean_image.binary')
        if exist(output_mean):
            shutil.rmtree(output_mean)
        command_mean = "{} {} {}".format(image_mean,ouput_lmdb_train,output_mean)
        args = shlex.split(command_mean)
