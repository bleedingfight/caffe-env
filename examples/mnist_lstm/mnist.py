...
class MnistInput(object):
    def __init__(self, data, folder):
        if data == "train":
            zX = os.path.join(folder,  'train-images-idx3-ubyte.gz')
            zY = os.path.join(folder,  'train-labels-idx1-ubyte.gz')
        elif data == "test":
            zX = os.path.join(folder,  't10k-images-idx3-ubyte.gz')
            zY = os.path.join(folder,  't10k-labels-idx1-ubyte.gz')
        else:
    ...
    def read(self, num=-1):  # -1 to read all
        ...
        with gzip.open(zX) as fX, gzip.open(zY) as fY:
            ...
            if nX != nY:
                raise ValueError("Inconsistent data and label files")
    ...
    def prepare_lstm(self, recur_steps):
        ...
        # rows = recur_steps * new_rows to make it simple
        assert depth == 1 and rows % recur_steps == 0
        ...
        # use transpose to keep the sub-frames of different batches aligned at same time step
        X = X.swapaxes(1, 2)
        X = X.reshape((num_batches,) + (recur_steps * batch_size,
                                        ) + frame_shape)  # flatten for CNN
...
if __name__ == "__main__":
    datas = expanduser('~/caffe/data/mnist')
    traininput = MnistInput("train", datas)
    trainX, trainY, trainC = traininput.prepare()
    testinput = MnistInput("test", datas)
    testX, testY, testC = testinput.prepare()
