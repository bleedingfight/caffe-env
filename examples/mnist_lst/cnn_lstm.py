...
def cnn_lstm(**args):
    ...
    n.pool1 = L.Pooling(n.relu2, kernel_h=1, kernel_w=2,
                        stride_h=1, stride_w=2, pool=P.Pooling.MAX)
    ...
    n.lstm = L.LSTM(n.fc1_reshape, n.clip,
                    recurrent_param=dict(num_output=n_output))

    n.lstm_last_step = L.Slice(n.lstm, slice_param=dict(
        axis=0, slice_point=T-1), ntop=2)[-1]
