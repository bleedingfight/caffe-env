...
class SequenceGenerator():
    ...
    def get_next_batch(self, truncate_at_exhaustion=True):
        ...
        batch = {}
        batch_indicators = np.zeros(
            (self.batch_stream_length, self.batch_num_streams))
        for name in self.substream_names:
            batch[name] = self.get_pad_value(
                name) * np.ones_like(batch_indicators)
        ...
        for t in range(self.batch_stream_length):
            ...
            for i in range(self.batch_num_streams):
                if not exhausted[i]:
                    ...
                    batch_indicators[t,
                                     i] = 0 if self.stream_indices[i] == 0 else 1
...
class HDF5SequenceWriter():
    ...
    def write_batch(self, stop_at_exhaustion=False):
        ...
        dataset = h5file.create_dataset(
            'cont', shape=cont_indicators.shape, dtype=cont_indicators.dtype)
        dataset[:] = cont_indicators
        dataset = h5file.create_dataset(
            'buffer_size', shape=(1,), dtype=np.int)
        ...
        for key, batch in batch_comps.iteritems():
            if self.verbose:
                for s in range(self.generator.batch_num_streams):
                    ...
                    print('batch %d, stream %s, index %d: ' %
                          (batch_index, key, s), stream)
            h5dataset = h5file.create_dataset(
                key, shape=batch.shape, dtype=batch.dtype)
            h5dataset[:] = batch
        h5file.close()
