...
def split_sentence(sentence):
    ...
    sentence = [s.lower() for s in SENTENCE_SPLIT_REGEX.split(
...
class CocoSequenceGenerator(SequenceGenerator):
    def __init__(self, coco, batch_num_streams, image_root, vocab=None,
        ...
        for image_id in split_ids:
            ...
            if os.path.isfile(image_path):
                ...
                num_missing += 1
                print('Warning (#%d): image not found: %s' %
                      (num_missing, image_path))
            num_total += 1
        ...
        for image_filename, metadata in known_images.iteritems():
            if not metadata['sentences']:
                num_no_sentences += 1
                print('Warning (#%d): image with no sentences: %s' %
                      (num_no_sentences, image_filename))
    ...
    def init_vocabulary(self, image_annotations, min_count=5):
        ...
        words_by_count = sorted(words_to_count.keys(),
                                key=lambda w: (-words_to_count[w], w))
        print('Initialized vocabulary with %d words; top 10 words:' %
              len(words_by_count))
    ...
    def dump_image_file(self, image_filename, dummy_image_filename=None):
        ...
        if dummy_image_filename is not None:
            print('Dumping image list with dummy labels to file: %s' %
